...
/** Нэг цэгийн хэмжилт */
/* --------------------------------------------------------------------------
   ЦЭГИЙН КОД — МОНГОЛ ТОВЧЛОЛООР

   Эх сурвалж цэгийг дүүргийн ЛАТИН товчлолоор дугаарладаг ("BGD-22",
   "KHD-19"). Дэлгэц дээр латин товчлол нь платформын "товчлол задална"
   дүрмийн эсрэг бөгөөд уншигчид ч харь (хэрэглэгч 2026-09-17: "цэгийн
   нэрүүд монгол товчлолоороо"). Зөвхөн ЗУРААСНЫ ӨМНӨХ угтварыг хөрвүүлнэ,
   дугаар хэвээр; толь бичигт байхгүй угтвар (2023 оны дугаарлалт, Төв
   аймгийн сум) байгаагаараа үлдэнэ — таамаглахгүй.

   ⚠ Хөрс, саармагжуулалт хоёулаа ЭНЭ функцээр — нэг цэг хоёр самбарт нэг
   нэртэй байх ёстой (BZD-16 = БЗД-16).
   -------------------------------------------------------------------------- */
...
export function mnPointCode(code: string): string {
  const m = /^([A-Za-z]+)(\s*-\s*.+)$/.exec(code.trim());
  if (!m) return code;
  const mn = DISTRICT_LATIN[m[1].toUpperCase()];
  return mn ? `${mn}${m[2].replace(/\s+/g, "")}` : code;
}
...
export async function fetchSoil(year: SoilYear): Promise<SoilData> {
  ...
  for (const f of json.features ?? []) {
    ...
    points.push({
      ...
      code: mnPointCode(String(a[s.code] ?? a.objectid)),
      lon: g.x,
      lat: g.y,
...
/**
 * ӨНГӨТ БИЧВЭРТ зориулсан хувилбар — одоо шатлалын ЦЭВЭР өнгө.
 *
 * Түүх: 2026-09-17-нд шатлалын шар, цайвар ногоон цагаан дэвсгэр дээр
 * уншигдахгүй байсан тул `--ink`-тэй 30% хольж байв. Гэвч тэр нь
 * гэрэл горимд тоог бор, бүдэг болгож, хэрэглэгч "харанхуйтай ижил
 * тод байгаасай" гэж буцаалгав (хэрэглэгчийн шийдвэр, 2026-09-17).
 * Тиймээс PLI өнгөт бичвэр ХОЁР горимд ЯГ ИЖИЛ — зурвас, цэгтэйгээ
 * нэг өнгө. Шар үзүүр (PLI ≈ 1.6) цагаан дээр сул уншигдана гэдгийг
 * хэрэглэгч мэдэж сонгосон; дахин холих хэрэг гарвал энд л нэг
 * газар өөрчилнө — бүх дуудагч энэ функцээр дамждаг.
 */
