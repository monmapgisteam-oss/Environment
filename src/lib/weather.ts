...
/* --------------------------------------------------------------------------
   САЛХИНЫ ХУРДНЫ АНГИЛАЛ

   ⚠⚠ ХЭЛТЭС ХУВААРИА ӨГСӨН (2026-09-21) тул салхи нь бусад
   хэмжигдэхүүнээс ЯЛГААТАЙ: тасралтгүй шатлал биш, НЭРТЭЙ ТАВАН
   АНГИЛАЛ. Хил нь 2 · 5 · 10 · 15 м/с.

   ⚠ Урьд нь гурван шаттай (0 `--data` · 8 `--ochre` · 15 `--clay`)
   тасралтгүй шатлал байсан бөгөөд `stopHex` нь ХАМГИЙН ОЙРЫН шатыг
   буцаадаг тул бодит хил нь 4 ба 11.5 м/с дээр буудаг байв — хэн ч
   тэр тоог сонгоогүй, тооцооны гаралт байлаа (хэрэглэгч "яагаад
   цэнхэр, шар хоёр өнгөтэй байгааг тайлбарла" гэж асууснаар илэрсэн).

   ⚠⚠ ӨНГӨ нь ХОЁР ТИЙШ АЖИЛЛАНА: `stopHex` (диаграм, радар, тоон
   бичвэр) хамгийн ойрын шатыг, газрын зураг (`gradeColor`) нь
   `interpolate linear`-ээр ХОЛИЛТЫГ хэрэглэдэг. Ангиллын хатуу хилийг
   хоёуланд нь зэрэг хүчинтэй болгохын тулд `bandStops` нь хил бүрийн
   ӨМНӨ өмнөх ангийн өнгийг давтаж бичнэ — шатлал нь шатан хэлбэртэй
   болж, холилт нь хилээс гадуур явахаа болино. Ингэснээр
   `wells/map.tsx`-ийг хөндөх шаардлагагүй (25 самбар хуваалцдаг).

   ⚠ Өнгө нь платформын гэр бүлээс: цайвар цэнхэр ба ногоон хоёр нь
   OKLCH дээр байгаа дохионуудтай ижил гэрэлтэлт, ханалттай (L 0.81,
   C 0.10–0.13), зөвхөн өнцгөөрөө ялгаатай.
   ⚠ Ногоон ↔ шар нь дейтеранопийн төөрөгдлийн хос боловч энд өнгө нь
   ГАНЦ суваг БИШ: радар дээр радиус, газрын зураг дээр цэгийн хэмжээ
   мөн хурдыг хэлнэ, дээр нь ангиллын НЭР бичвэрээр гарна.
   -------------------------------------------------------------------------- */
...
/** Салхины хурдны ангилал — хэлтсийн өгсөн хуваарь */
export const WIND_BANDS: { from: number; label: string; hex: string }[] = [
  { from: 0, label: "Сул", hex: SKY },
  { from: 2, label: "Зөөлөн", hex: LEAF },
  { from: 5, label: "Дунд зэрэг", hex: OCHRE },
  { from: 10, label: "Хүчтэй", hex: FLAME },
  { from: 15, label: "Маш хүчтэй", hex: CLAY },
];

/** Утга → ангилал. Хил нь ДООД захаараа хамаарна (2 нь "Зөөлөн") */
export function windBand(v: number | null) {
  if (v == null) return null;
  let hit = WIND_BANDS[0];
  for (const b of WIND_BANDS) if (v >= b.from) hit = b;
  return hit;
}

/**
 * Ангиллуудыг ШАТАН шатлал болгоно: хил бүрийн өмнө өмнөх ангийн өнгө
 * давтагдсанаар холилт ч, ойрын шатны хайлт ч ижил үр дүн өгнө.
 */
function bandStops(
  bands: { from: number; hex: string }[],
): [number, string][] {
  const out: [number, string][] = [];
  for (const [i, b] of bands.entries()) {
    if (i > 0) out.push([b.from - 0.01, bands[i - 1].hex]);
    out.push([b.from, b.hex]);
  }
  return out;
}
...
export const MEASURES: Measure[] = [
  ...
  },
  {
    ...
    digits: 1,
    of: (o) => o.wind,
    /* ⚠ Ангилалтай ЦОРЫН ГАНЦ хэмжигдэхүүн — дээрх тайлбарыг үзнэ үү */
    stops: bandStops(WIND_BANDS),
  },
  {
    id: "pressure",
